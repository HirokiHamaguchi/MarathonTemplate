 ...
 * @file viewer.hpp
 * @author hari64
 * @brief viewer for debug
 * @version 0.1
 * @copyright Copyright (c) 2022
 * @note /usr/bin/g++-10 -x c++-header -g -std=c++17 -Dhari64 -Wall -Wextra
 * -fdiagnostics-color=always -D_GLIBCXX_DEBUG viewer.hpp
 */
#include <bits/stdc++.h>
...
void _debug(int n, std::string S, const H& h, const T&... t) {
    if (n != -1 && S.size() >= 8 &&
        ...
        std::cerr << "\033[1;32m" << n << "\033[0m: \033[1;36m" << S
                  << "\033[0m = ";
    }
    v(h, sizeof...(t) ? "," : "\n");
    _debug(-1, "", t...);
